...
class _FaceScreenState extends State<FaceScreen> {
  ...
  Future<void> _pick(ImageSource source) async {
    ...
    try {
      final result = await _analysisService.analyzeFace(file.path);
      setState(() {
        _result = result;
        _isAnalyzing = false;
        if (result == null) {
  ...
  Future<void> _save() async {
    ...
    if (mounted && id.isNotEmpty) {
      ...
      context.push(
        '/result',
        extra: {'type': 'face', 'result': _result, 'sessionId': id},
      );
    }
  }
            ...
            GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: AppColors.faceColor.withOpacity(0.08),
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(
                    color: _imagePath != null ? AppColors.faceColor : AppColors.faceColor.withOpacity(0.3),
                ...
                child: _imagePath == null
                    ? Column(
                        ...
                        children: [
                          Icon(Icons.add_photo_alternate_outlined, size: 48, color: AppColors.faceColor.withOpacity(0.6)),
                          const SizedBox(height: 12),
                          Text('Appuyez pour choisir une image', style: TextStyle(color: AppColors.faceColor.withOpacity(0.7))),
                        ],
                      )
                    : ClipRRect(
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    ...
                    style: OutlinedButton.styleFrom(foregroundColor: AppColors.faceColor, side: BorderSide(color: AppColors.faceColor)),
                  ),
                ),
            ...
            if (_isAnalyzing)
              ...
              Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  _FaceResultCard(result: _result!),
                  const SizedBox(height: 16),
                  ElevatedButton.icon(
                    icon: const Icon(Icons.save_rounded),
                    label: const Text('Sauvegarder dans l\'historique'),
                    onPressed: _save,
                    style: ElevatedButton.styleFrom(backgroundColor: AppColors.faceColor),
                  ),
                ],
              ),
          ],
...
class _FaceResultCard extends StatelessWidget {
  ...
  const _FaceResultCard({required this.result});

  Color _getRpeColor(int rpe) {
    if (rpe <= 3) return AppColors.effortLow;
    if (rpe <= 5) return AppColors.effortMed;
    if (rpe <= 7) return AppColors.effortHigh;
    return AppColors.effortMax;
  }

  @override
  Widget build(BuildContext context) {
    final color = _getRpeColor(result.rpeScore);
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
          ...
          if (result.highFatigueAlert)
            Container(
              ...
              child: Row(children: [
                ...
                const Expanded(child: Text('⚠️ Fatigue extrême détectée !', style: TextStyle(color: AppColors.error, fontWeight: FontWeight.bold))),
              ]),
            ),
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('RPE Score:', style: TextStyle(fontWeight: FontWeight.bold)),
            Text('${result.rpeScore}/10', style: TextStyle(color: color, fontWeight: FontWeight.bold, fontSize: 24)),
          ]),
          const SizedBox(height: 4),
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('Effort:', style: TextStyle(fontWeight: FontWeight.bold)),
            Text(result.effortLabel, style: TextStyle(color: color)),
          ]),
          const Divider(),
          const Text('Expressions détectées:', style: TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: 4),
          ...result.expressions.map((exp) => Padding(
            padding: const EdgeInsets.only(bottom: 4),
            child: Row(children: [
              Icon(Icons.face, size: 14, color: AppColors.faceColor),
              const SizedBox(width: 4),
              Expanded(child: Text(exp, style: const TextStyle(fontSize: 12))),
            ]),
          )),
          const Divider(),
          Text(result.fatigueAdvice, style: const TextStyle(fontSize: 12, fontStyle: FontStyle.italic)),
        ],
      ),
    );
  }
