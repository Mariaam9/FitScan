...
class _PoseScreenState extends State<PoseScreen> {
  ...
  Future<void> _pick(ImageSource source) async {
    ...
    try {
      final result = await _analysisService.analyzePose(file.path);
      if (mounted) {
        setState(() {
          _result = result;
          _isAnalyzing = false;
  ...
  Future<void> _save() async {
    ...
    if (mounted && id.isNotEmpty) {
      ...
      context.push(
        '/result',
        extra: {'type': 'pose', 'result': _result, 'sessionId': id},
      );
    }
  }
              ...
              GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    color: AppColors.poseColor.withOpacity(0.08),
                    borderRadius: BorderRadius.circular(20),
                    border: Border.all(
                      color: _imagePath != null
                          ? AppColors.poseColor
                          : AppColors.poseColor.withOpacity(0.3),
                      ...
                      ? Column(
                          ...
                          children: [
                            Icon(Icons.add_photo_alternate_outlined,
                                size: 48,
                                color: AppColors.poseColor.withOpacity(0.6)),
                            const SizedBox(height: 12),
                            Text('Appuyez pour choisir une image',
                                style: TextStyle(
                                    color: AppColors.poseColor.withOpacity(0.7))),
                          ],
                        )
                      : ClipRRect(
                  ...
                  Expanded(
                    child: OutlinedButton.icon(
                      ...
                      style: OutlinedButton.styleFrom(
                          foregroundColor: AppColors.poseColor,
                          side: BorderSide(color: AppColors.poseColor)),
                    ),
                  ),
              ...
              if (_isAnalyzing)
                ...
                Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    _PoseResultCard(result: _result!),
                    const SizedBox(height: 16),
                    ElevatedButton.icon(
                      ...
                      style: ElevatedButton.styleFrom(
                          backgroundColor: AppColors.poseColor),
                    ),
                  ],
                ),
            ],
...
class _PoseResultCard extends StatelessWidget {
  ...
  const _PoseResultCard({required this.result});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: AppColors.poseColor.withOpacity(0.3)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          if (result.corrections.isNotEmpty) ...[
            ...
            ...result.corrections.map((c) => Padding(
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.fitness_center, size: 14, color: AppColors.success),
                      const SizedBox(width: 4),
